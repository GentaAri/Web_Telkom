...
$sql = "SELECT * FROM tb_print2 where id=1";
...
$no=1;
$query=mysqli_query($koneksi, "select * from tb_print2");
...
while( $pdf->GetStringWidth( $d['nama'] ) > 45 ){
    ...
    $pdf->SetFont( 'Gadugi', '', $x );  // Set new font size
}
...
$pdf->Output('Surat Tugas Opr.pdf', 'I'); 
$query=mysqli_query($koneksi, "TRUNCATE TABLE  tb_print2");
ob_end_flush();
?>