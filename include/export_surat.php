...
$excel->getProperties()->setCreator('TelkomAksess')
					   ...
					   ->setTitle("Data Surat SPBU")
					   ->setSubject("Surat SPBU")
					   ->setDescription("Laporan Semua Data Surat SPBU")
					   ->setKeywords("Data Surat SPBU");
...
$excel->setActiveSheetIndex(0)->setCellValue('A1', "Data Surat SPBU"); // Set kolom A1 dengan tulisan "DATA SISWA"
...
$excel->setActiveSheetIndex(0)->setCellValue('D3', "JABATAN MGR"); // Set kolom C3 dengan tulisan "NAMA"
$excel->setActiveSheetIndex(0)->setCellValue('E3', "NAMA"); // Set kolom D3 dengan tulisan "JENIS KELAMIN"
$excel->setActiveSheetIndex(0)->setCellValue('F3', "NIK"); // Set kolom E3 dengan tulisan "TELEPON"
$excel->setActiveSheetIndex(0)->setCellValue('G3', "JABATAN"); // Set kolom F3 dengan tulisan "ALAMAT"
$excel->setActiveSheetIndex(0)->setCellValue('H3', "TANGGAL MULAI");
$excel->setActiveSheetIndex(0)->setCellValue('I3', "TANGGAL SELESAI");
$excel->setActiveSheetIndex(0)->setCellValue('J3', "PENUGASAN");
$excel->setActiveSheetIndex(0)->setCellValue('K3', "ID SPBU");
$excel->setActiveSheetIndex(0)->setCellValue('L3', "ALAMAT SPBU");
...
$sql = $pdo->prepare("SELECT * FROM tb_surat");
$sql->execute(); // Eksekusi querynya
 setlocale(LC_ALL, 'id-ID', 'id_ID');
$no = 1; // Untuk penomoran tabel, di awal set dengan 1
$numrow = 4; // Set baris pertama untuk isi tabel adalah baris ke 4
while($data = $sql->fetch()){ // Ambil semua data dari hasil eksekusi $sql
	$date1=$data['tanggal_mulai'];
	$date2=$data['tanggal_selesai'];
	...
	$excel->setActiveSheetIndex(0)->setCellValue('D'.$numrow, $data['jabatan_mgr']);
	...
	$excel->setActiveSheetIndex(0)->setCellValue('H'.$numrow, strftime("%A, %d %B %Y", strtotime($date1)));
	$excel->setActiveSheetIndex(0)->setCellValue('I'.$numrow, strftime("%A, %d %B %Y", strtotime($date2)));
	$excel->setActiveSheetIndex(0)->setCellValue('J'.$numrow, $data['penugasan']);
	$excel->setActiveSheetIndex(0)->setCellValue('K'.$numrow, $data['id_spbu']);
...
$excel->getActiveSheet(0)->setTitle("Data Surat SPBU");
$excel->setActiveSheetIndex(0);

// Proses file excel
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment; filename="Data Surat SPBU.xlsx"'); // Set nama file excel nya
